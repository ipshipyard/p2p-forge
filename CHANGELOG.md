...
- 🛠 The optional DynamoDB datastore now runs on the current AWS SDK for Go. Operators using `database-type dynamo` should double-check that their AWS credentials still get picked up; the usual `AWS_REGION`, `AWS_ACCESS_KEY_ID`, and `AWS_SECRET_ACCESS_KEY` env vars keep working. See the [go-ds-dynamodb v0.3.0 release notes](https://github.com/ipfs/go-ds-dynamodb/releases/tag/v0.3.0) for details.
