...
### Added
- ✨ New `/v2` registration API that authenticates with [HTTP Message Signatures (RFC 9421)](https://www.rfc-editor.org/rfc/rfc9421) plus [Digest Fields (RFC 9530)](https://www.rfc-editor.org/rfc/rfc9530) over an Ed25519 `did:key`, so a client can register without a libp2p HTTP stack. A node proves it controls a real endpoint either with a signed proof served at `/.well-known/autotls/<did:key>` (no libp2p) or with the libp2p dialback. The `/v1` PeerID-auth API is unchanged and runs alongside it. Clients opt in with `client.WithRegistrationAPIVersion`. See [docs/registration-v2.md](docs/registration-v2.md).
